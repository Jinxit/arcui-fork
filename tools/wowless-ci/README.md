...
3. **Run the matcher in dry-run mode** (just observe — don't fail on stale):
   ...
   This shows exactly which entries are stale, vanished, or unexpected.

4. **For each stale entry that still reproduces:** update `acceptedOnBuild`
   and `acceptedOnVersion` to the new values.

5. **For each vanished entry:** remove it.  Note in the PR what changed
   (wowless fix, upstream patch, etc.).

6. **For each unexpected entry:** decide:
...
7. Commit updated `accepted-errors.yaml` and push.

...
local collapsedSections = {
  ...
  autoSwitch = true,
  autoShare = true,
  iconAnchor = true,  -- [FORK] icon-to-icon anchor section (issue #40)
}
  ...
  local appearanceOptions = {
    ...
    args = {
      ...
      iconAnchorHeader = {
        type = "toggle",
        name = "Anchor to Icon",
        desc = "Click to expand/collapse. Attach this icon on top of another tracker icon.",
        dialogControl = "CollapsibleHeader",
        get = function() return not collapsedSections.iconAnchor end,
        set = function(info, value) collapsedSections.iconAnchor = not value end,
        order = 9.9,
        width = "full",
        hidden = function() return GetSelectedConfig() == nil or IsBarMode() end
      },
      iconAnchorEnable = {
        type = "toggle",
        name = "Enable",
        desc = "Anchor this icon on top of another tracker icon. Disables free-dragging while active.",
        get = function()
          local cfg = GetSelectedConfig()
          return cfg and cfg.display.anchorToTracker or false
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if not cfg then return end
          cfg.display.anchorToTracker = value
          if not value and ns.TrackerAnchors then
            local _, barNum = GetSelectedBarType()
            if barNum then ns.TrackerAnchors.UnregisterSource("buffBarIcon", tostring(barNum)) end
          end
          RefreshBar()
        end,
        order = 9.91,
        width = 0.7,
        hidden = function() return GetSelectedConfig() == nil or IsBarMode() or collapsedSections.iconAnchor end
      },
      iconAnchorTarget = {
        type = "select",
        name = "Target Icon",
        desc = "Which icon to anchor on top of. Buff bar icons and Arc Auras can be sources or targets; CDM icons are targets only.",
        values = function()
          local _, barNum = GetSelectedBarType()
          if not ns.TrackerAnchors or not ns.TrackerAnchors.BuildTargetValues then return {} end
          local vals = ns.TrackerAnchors.BuildTargetValues("buffBarIcon", barNum and tostring(barNum) or "")
          vals[""] = "— Select target —"
          return vals
        end,
        sorting = function()
          local _, barNum = GetSelectedBarType()
          if not ns.TrackerAnchors or not ns.TrackerAnchors.BuildTargetValues then return {} end
          local _, sorting = ns.TrackerAnchors.BuildTargetValues("buffBarIcon", barNum and tostring(barNum) or "")
        ...
        get = function()
          ...
          return ns.TrackerAnchors and ns.TrackerAnchors.EncodeTargetValue(
            cfg.display.anchorTargetKind, cfg.display.anchorTargetKey) or ""
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if not cfg then return end
          ...
          -- Cycle check
          if kind and kind ~= "" and ns.TrackerAnchors and
             ns.TrackerAnchors.HasCycle("buffBarIcon", barNum and tostring(barNum) or "", kind, key) then
            print("|cffFF6600[ArcUI]|r Anchor rejected: would create a cycle.")
            return
          end
          cfg.display.anchorTargetKind = kind or ""
          cfg.display.anchorTargetKey  = key  or ""
          RefreshBar()
        end,
        order = 9.92,
        width = "full",
        hidden = function()
          local cfg = GetSelectedConfig()
          return not cfg or cfg.display.displayType ~= "icon" or
                 collapsedSections.iconAnchor or not cfg.display.anchorToTracker
        end
      },
      iconAnchorSourcePoint = {
        type = "select",
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return cfg and cfg.display.anchorSourcePoint or "CENTER"
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then cfg.display.anchorSourcePoint = value RefreshBar() end
        end,
        order = 9.93,
        width = 1.0,
        hidden = function()
      ...
      iconAnchorTargetPoint = {
        ...
        get = function()
          local cfg = GetSelectedConfig()
          return cfg and cfg.display.anchorTargetPoint or "CENTER"
        end,
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then cfg.display.anchorTargetPoint = value RefreshBar() end
        end,
        order = 9.94,
        width = 1.0,
        hidden = function()
      ...
      iconAnchorOffsetX = {
        ...
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then cfg.display.anchorOffsetX = value RefreshBar() end
        end,
        order = 9.95,
        width = 1.0,
        hidden = function()
      ...
      iconAnchorOffsetY = {
        ...
        set = function(info, value)
          local cfg = GetSelectedConfig()
          if cfg then cfg.display.anchorOffsetY = value RefreshBar() end
        end,
        order = 9.96,
        width = 1.0,
        hidden = function()
      ...
      -- [FORK] begin: 9-point anchor preset + advanced selectors; replaces anchorPoint dropdown (issue #14)
      anchorPosition = {
        type = "select",
        name = "Position",
      ...
      matchGroupWidth = {
        type = "toggle",
        name = "Match Size",
        desc = "Automatically resize the bar to match the group container.\n\nMatches the bar's primary axis: width for horizontal bars, height for vertical bars.", -- [FORK] updated desc (issue #14)
        get = function()
          local cfg = GetSelectedConfig()
      ...
      -- [FORK] begin: percentage size adjustment (issue #14)
      matchWidthAdjustPct = {
        type = "range",
        name = "Size Adjust %",
